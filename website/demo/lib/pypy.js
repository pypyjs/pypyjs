...
function PyPyJS(opts) {
  ...
  this.ready = new Promise((function(resolve, reject) {
    ...
    .then((function(xhr) {
      ...
      this._allModules = JSON.parse(xhr.responseText);
...
PyPyJS.prototype._makeLoadModuleData = function _makeLoadModuleData(name) {
  return (function() {
    ...
    console.log("fetching module data: " + modfile);
    var p = this.fetch("modules/" + modfile)
    .then((function(xhr) {
      var contents = xhr.responseText;
      // Create the containing directory first.
      var moddir = modfile.split("/").slice(0, -1).join("/")
      try {
        Module.FS_createPath("/lib/pypyjs/lib_pypy", moddir, true, false);
      } catch (e) { }
      // Now we can safely create the file.
      var fullpath = "/lib/pypyjs/lib_pypy/" + modfile;
      Module.FS_createDataFile(fullpath, "", contents, true, false, true);
